
export const historyQuestions = [
  {
    question: "Who was the first President of the United States?",
    options: ["Thomas Jefferson", "George Washington", "John Adams", "James Madison"],
    correctAnswer: "George Washington",
  },
  {
    question: "In which year did Christopher Columbus first reach the Americas?",
    options: ["1492", "1520", "1456", "1510"],
    correctAnswer: "1492",
  },
  {
    question: "Who was the leader of the Soviet Union during World War II?",
    options: ["Vladimir Putin", "Joseph Stalin", "Nikita Khrushchev", "Leon Trotsky"],
    correctAnswer: "Joseph Stalin",
  },
  {
    question: "What event marked the beginning of World War I?",
    options: ["The sinking of Lusitania", "The assassination of Franz Ferdinand", "The Battle of the Somme", "The Treaty of Versailles"],
    correctAnswer: "The assassination of Franz Ferdinand",
  },
  {
    question: "Who was the first woman to win a Nobel Prize?",
    options: ["Marie Curie", "Rosa Parks", "Emmeline Pankhurst", "Jane Addams"],
    correctAnswer: "Marie Curie",
  },
  {
    question: "Which ancient civilization built the pyramids in Egypt?",
    options: ["Mesopotamians", "Greeks", "Romans", "Ancient Egyptians"],
    correctAnswer: "Ancient Egyptians",
  },
  {
    question: "In which year did the American Civil War end?",
    options: ["1863", "1865", "1870", "1861"],
    correctAnswer: "1865",
  },
  {
    question: "Who wrote the 'Communist Manifesto' with Karl Marx?",
    options: ["Friedrich Engels", "Vladimir Lenin", "Leon Trotsky", "Mao Zedong"],
    correctAnswer: "Friedrich Engels",
  },
  {
    question: "Which famous speech begins with the words 'I have a dream'?",
    options: ["John F. Kennedy", "Malcolm X", "Martin Luther King Jr.", "Nelson Mandela"],
    correctAnswer: "Martin Luther King Jr.",
  },
  {
    question: "What is the significance of the Magna Carta in English history?",
    options: ["It established the Church of England", "It granted women the right to vote", "It limited the power of the monarchy", "It abolished slavery"],
    correctAnswer: "It limited the power of the monarchy",
  },
  {
    question: "Who was the leader of the Allied forces during World War II?",
    options: ["Winston Churchill", "Franklin D. Roosevelt", "Joseph Stalin", "Dwight D. Eisenhower"],
    correctAnswer: "Dwight D. Eisenhower",
  },
  {
    question: "Which ancient empire was ruled by Julius Caesar, Augustus, and Nero?",
    options: ["Persian Empire", "Roman Empire", "Byzantine Empire", "Ottoman Empire"],
    correctAnswer: "Roman Empire",
  },
  {
    question: "In which year did the French Revolution begin?",
    options: ["1776", "1789", "1804", "1832"],
    correctAnswer: "1789",
  },
  {
    question: "Who was the first woman to receive a Ph.D. in computer science and make significant contributions to the development of computer programming?",
    options: ["Ada Lovelace", "Grace Hopper", "Margaret Hamilton", "Barbara Liskov"],
    correctAnswer: "Barbara Liskov",
  },
  {
    question: "What event led to the start of the Korean War in 1950?",
    options: ["The Cuban Missile Crisis", "The invasion of South Korea by North Korea", "The Vietnam War", "The Suez Crisis"],
    correctAnswer: "The invasion of South Korea by North Korea",
  },
  {
    question: "Who was the first Emperor of China?",
    options: ["Mao Zedong", "Qin Shi Huang", "Sun Yat-sen", "Emperor Wu"],
    correctAnswer: "Qin Shi Huang",
  },
  {
    question: "Which European city was divided by the Berlin Wall during the Cold War?",
    options: ["Paris", "London", "Berlin", "Rome"],
    correctAnswer: "Berlin",
  },
  {
    question: "Who was the founder of the Mongol Empire?",
    options: ["Kublai Khan", "Genghis Khan", "Attila the Hun", "Tamerlane"],
    correctAnswer: "Genghis Khan",
  },
  {
    question: "What was the main cause of the Irish Potato Famine in the mid-19th century?",
    options: ["Drought", "Blight", "War", "Flood"],
    correctAnswer: "Blight",
  },
  {
    question: "In which year did the United States declare its independence from Great Britain?",
    options: ["1776", "1789", "1804", "1832"],
    correctAnswer: "1776",
  },
  {
    question: "Who was the first Emperor of China?",
    options: ["Qin Shi Huang", "Han Wudi", "Emperor Wu", "Li Shimin"],
    correctAnswer: "Qin Shi Huang",
  },
  {
    question: "Which famous explorer sailed around the world in the 16th century?",
    options: ["Marco Polo", "Ferdinand Magellan", "Vasco da Gama", "Christopher Columbus"],
    correctAnswer: "Ferdinand Magellan",
  },
  {
    question: "During which war did the Battle of Gettysburg take place?",
    options: ["American Revolutionary War", "World War I", "American Civil War", "World War II"],
    correctAnswer: "American Civil War",
  },
  {
    question: "Who was the Egyptian queen known for her relationship with Julius Caesar and Mark Antony?",
    options: ["Nefertiti", "Hatshepsut", "Cleopatra", "Isis"],
    correctAnswer: "Cleopatra",
  },
  {
    question: "Which ancient civilization is credited with inventing democracy?",
    options: ["Roman Empire", "Ancient Persia", "Ancient Greece", "Mesopotamia"],
    correctAnswer: "Ancient Greece",
  },
  {
    question: "What was the main cause of the French Revolution?",
    options: ["Economic inequality", "Religious conflicts", "Foreign invasions", "Monarchical succession"],
    correctAnswer: "Economic inequality",
  },
  {
    question: "Who was the first woman to fly solo across the Atlantic Ocean?",
    options: ["Amelia Earhart", "Bessie Coleman", "Harriet Quimby", "Jacqueline Cochran"],
    correctAnswer: "Amelia Earhart",
  },
  {
    question: "Which emperor is known for building the Great Wall of China?",
    options: ["Han Wudi", "Kangxi", "Qin Shi Huang", "Ming Taizu"],
    correctAnswer: "Qin Shi Huang",
  },
  {
    question: "What event led to the start of the Protestant Reformation?",
    options: ["Council of Trent", "Peace of Augsburg", "Diet of Worms", "Edict of Nantes"],
    correctAnswer: "Diet of Worms",
  },
  {
    question: "Who was the leader of the Bolsheviks during the Russian Revolution?",
    options: ["Vladimir Lenin", "Joseph Stalin", "Leon Trotsky", "Nikita Khrushchev"],
    correctAnswer: "Vladimir Lenin",
  },
  {
    question: "Who was the 16th President of the United States?",
    options: ["Abraham Lincoln", "Andrew Johnson", "Ulysses S. Grant", "James Buchanan"],
    correctAnswer: "Abraham Lincoln",
  },
  {
    question: "Which ancient wonder of the world was located in Babylon?",
    options: ["The Hanging Gardens", "The Great Pyramid of Giza", "The Statue of Zeus at Olympia", "The Temple of Artemis"],
    correctAnswer: "The Hanging Gardens",
  },
  {
    question: "Who wrote the 'Declaration of Independence' for the United States?",
    options: ["Thomas Jefferson", "John Adams", "George Washington", "Benjamin Franklin"],
    correctAnswer: "Thomas Jefferson",
  },
];